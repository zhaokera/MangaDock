...
优酷网站特点：
- URL 格式: https://v.youku.com/v_show/id_{video_id}.html
- 使用 flv/ts 视频格式
- 需要处理反爬机制
"""
...
# URL 模式
_YOUKU_PATTERN = re.compile(r'v\.youku\.com/v_show/id_[a-zA-Z0-9=]+\.html')
_YOUKU_TV_PATTERN = re.compile(r'tv\.youku\.com/show/[a-zA-Z0-9]+')

# 优酷 API
_YOUKU_API_BASE = "https://v.youku.com"
...
class YoukuCrawler(BaseCrawler):
    ...
    URL_PATTERNS = [
        ...
        r"tv\.youku\.com/show/[a-zA-Z0-9]+",
    ]

    def _extract_ids(self, url: str) -> tuple:
        """从 URL 提取 video_id"""
        # 尝试提取 v_show 格式
        match = re.search(r'v\.youku\.com/v_show/id_([a-zA-Z0-9=]+)\.html', url)
        if match:
            return None, match.group(1)

        # 尝试提取 show 格式
        match = re.search(r'tv\.youku\.com/show/([a-zA-Z0-9]+)', url)
        if match:
            return None, match.group(1)
